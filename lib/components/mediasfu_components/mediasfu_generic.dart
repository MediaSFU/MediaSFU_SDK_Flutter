...
import '../background_components/virtual_background_types.dart'
    show VirtualBackground, BackgroundType;
import '../background_components/background_processor_service.dart'
    show BackgroundProcessorService;
import '../background_components/native_virtual_background.dart'
    ...
    if (hasChanged) {
      ...
      if (videoAlreadyOn.value == true) {
        ...
        if (NativeVirtualBackground.isSupported) {
          ...
          // If native VB succeeded, stop any fallback processor that might be running
          if (useNative) {
            try {
              // Small delay to allow in-flight frames to complete gracefully
              await Future.delayed(const Duration(milliseconds: 100));
              final processorService = BackgroundProcessorService();
              await processorService.stop();
            } catch (e) {
              // Ignore errors from stopping fallback
            }
          }
        }
