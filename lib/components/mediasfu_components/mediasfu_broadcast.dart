...
///
/// ### Parameters:
/// - `options` (`MediasfuBroadcastOptions`): Configuration options for setting up the widget.
///
/// ### Example Usage:
/// ```dart
/// MediasfuBroadcast(
...
///      updateSourceParameters: (MediasfuParameters? parameters) { myUpdateFunction(parameters); },
///     returnUI: true,
...
class _MediasfuBroadcastState extends State<MediasfuBroadcast> {
  ...
  Future<ResponseJoinRoom> joinRoom(
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('Error joining room: $error $stackTrace');
      }
  ...
  final ValueNotifier<EventType> eventType = ValueNotifier(EventType.broadcast);
...
// Recording States
  ...
  double mainHeightWidth = 100;
  final ValueNotifier<double> prevMainHeightWidth = ValueNotifier(100);
...
// Update functions
  ...
  void initState() {
    ...
    if (widget.options.returnUI != null &&
        widget.options.returnUI == false &&
        widget.options.updateSourceParameters != null) {
      try {
        widget.options.sourceParameters = mediasfuParameters;
        widget.options.updateSourceParameters!(widget.options.sourceParameters);
      } catch (error) {
        if (kDebugMode) {
